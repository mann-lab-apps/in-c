...
`Classes`, `Chromatics 앱` 순서로 노출한다. Works, Concerts, Creators는 정적 MVP
페이지를 갖고, Classes는 상세 페이지 전까지 `index.html#product-map`으로 연결한다.

Works, Concerts, Creators 페이지는 `site/product-data.js`를 공유한다. Supabase
