...
- Works: 작품, 악보, Columns, 공연, Creator를 묶는 관계 허브.
- Concerts: 공연 전에 들을 지점을 보여주는 프리뷰 영역.
- Creators: 작곡가, 연주자, 기획자를 작품과 콘텐츠로 연결하는 영역.
- Classes: 감상 클래스와 레슨을 작품, Columns, Creator와 연결하는 정보형 영역.
- Chromatics 앱: in C 전체가 아니라 MusicXML과 단선율 사보를 담당하는 무료 창작 도구.
- 향후 Classes, Communities/Connects로 확장할 수 있는 관계 구조.

## Navigation

공통 header는 `Columns`, `Compositions`, `Works`, `Concerts`, `Creators`,
`Classes`, `Chromatics 앱` 순서로 노출한다. Works, Concerts, Creators, Classes는
정적 MVP 페이지를 갖는다.

Works, Concerts, Creators, Classes 페이지는 `site/product-data.js`를 공유한다. Supabase
