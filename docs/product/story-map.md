...
- 지원됨: 제목/부제목 수정, 박자표 선택, 조표 선택.
- 부분 지원: 생성 후 박자표/조표 변경, full-measure rest의 표시와 실제 박자
  길이 분리.
- 부족함: 새 악보 설정 흐름의 완성도와 오류 복구 안내.
...
- 작성한 악보를 나중에 다시 열거나, 다른 사보 도구와 주고받으려는 사용자.
- 배포된 앱에서 결과물을 파일로 남기고 공유하려는 사용자.

목표:

- 작성한 악보를 프로젝트 파일로 보관하고, MusicXML 또는 PDF로 내보낸다.
...
3. MusicXML로 내보낸다.
4. 필요한 경우 PDF로 변환한다.
5. 배포 빌드에서도 같은 흐름이 동작하는지 확인한다.

기대 결과:

- 다시 열었을 때 같은 음악 의미와 표기가 유지된다.
- MusicXML로 다른 도구와 주고받을 수 있다.
- 프로젝트 저장, MusicXML 내보내기, PDF 변환은 사용자가 구분할 수 있는 별도
  행동으로 제공된다.

현재 기준:

- 프로젝트 파일을 저장하고 다시 열 수 있어야 한다.
- 프로젝트 파일은 `.incproj` 컨테이너로 설계하며, MusicXML과 역할을 구분한다.
- MusicXML로 가져오기와 내보내기를 할 수 있어야 한다.
- PDF 변환은 프로젝트 저장과 구분되는 내보내기 기능으로 다룬다.
- 배포 빌드에서도 저장과 내보내기 흐름이 동작해야 한다.

지원 상태:

- 지원됨: MusicXML 가져오기와 내보내기 기본 흐름, PDF 변환, 파일 작업 UI의
  저장/내보내기 용어 분리.
- 부분 지원: 프로젝트 파일 포맷과 자동저장 전략, 배포 빌드에서의 파일 흐름.
- 부족함: 프로젝트 저장/열기 구현, 자동저장 복구 UX.

검증 연결:

- `npm test`: MusicXML 왕복 후 ID와 무관한 음악 의미 동등성 검증.
- `npm run verify:package`: 패키징된 앱 smoke test.
