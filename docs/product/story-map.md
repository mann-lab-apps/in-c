...
- 기존 MVP 사용자 여정의 전체 맥락을 참고한다.
- Gherkin 인수 시나리오로 아직 옮겨지지 않은 기능 기준을 임시로 확인한다.
- 새 기준이나 변경된 기준은 `docs/product/acceptance/*.feature`로 옮기는 것을
  우선한다.

AI Agent가 작업 전후에 Gherkin 인수 시나리오를 확인하고 갱신하는 절차는
`docs/product/agent-workflow.md`에 정리한다.

## 관리 원칙

- 이 문서는 legacy 맥락 문서로 유지한다.
- 변경 배경, 논의, 히스토리는 GitHub 이슈와 PR에 남긴다.
- 새 기능 기준은 가능하면 Gherkin `.feature` 파일에 추가한다.
- 작업 중 기준이 바뀌면 해당 이슈 댓글과 `.feature` 파일을 함께 보강한다.
- 이 문서와 `.feature` 파일이 충돌하면 `.feature` 파일을 우선하고, 필요하면
  이 문서를 정리한다.
...
## Legacy 스토리 영향도 확인 규칙

이 섹션은 Gherkin 인수 시나리오 체계로 전환하기 전 사용하던 규칙이다. 새 이슈와
PR에서는 `docs/product/acceptance/*.feature`와 `docs/product/agent-workflow.md`의
`인수 시나리오` 규칙을 우선한다.
