...
| 라이브러리 | 제목/작곡가/태그/메모 | 양쪽 기본 | MVP | 낮음 | 2차 구현: SharedPreferences 기반 편집 UI, comma-separated 태그. viewer 안에서도 `악보 정보 편집`으로 가져온 악보의 title/composer/tag/collection/group/rating/custom field를 바로 정리 가능 |
