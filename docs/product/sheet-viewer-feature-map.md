...
스토어/런처 표시 이름은 현재 `Clef`를 유지한다. `in Clef`, `in C - Clef`, 단독 `Clef` 후보는
기존 음악 앱과의 구분 가능성을 보고 출시 전 별도 결정한다. 패키지명과 내부 코드 네임은 배포
연속성을 위해 Clef를 유지한다.
...
| 음악 도구 | 튜너 | Piascore 참고/차별화 | MVP | 높음 | 구현됨: `record` PCM stream, autocorrelation detector, RMS gate/confidence, median smoothing, profile별 no-signal debounce, octave jump 완화, note hysteresis, frequency-to-note/cents 계산, Chromatic/Target mode, Guitar standard/Drop D/DADGAD/half-step down/7-string, Bass/5-string bass, Ukulele, Mandolin, Strings, Bb/Eb/F preset, custom target/preset 저장, target lock, sharp/flat 표기 선택, A4 저장/440-442 quick action/history/보정 제안, adaptive noise floor 1차, 입력강도 bar, LED flat/center/sharp strip, 소리 작음/잡는 중/낮음/높음/맞음 feedback, needle damping/in-tune hold. synthetic sine/noise/time-series test 통과. 실기기 정확도/latency 검증 필요 |
...
- 크로매틱/타겟 튜너. `record` 기반 microphone PCM stream, autocorrelation pitch detector, RMS
  gate, confidence, median smoothing, no-signal debounce, octave guard, note hysteresis,
  Concert/Bb/Eb/F/Strings/Guitar/Bass 표시, 악기별 tuning preset, target 기준 cents 계산,
  custom tuning target/preset 저장, sharp/flat 표기 선택, 안정적인 A 입력 기반 A4 보정 제안,
  상용 튜너형 feedback label/meter/LED strip/input power bar와 needle damping/in-tune hold를
  붙였다. Android 태블릿 실기기 정확도/latency 검증은 후속이다.
