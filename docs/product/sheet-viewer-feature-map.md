...
| 라이브러리 | 정렬/필터 | MobileSheets 지원 | MVP | 낮음 | 14차 구현: 최근 열기/제목/작곡가/가져온 날짜 정렬, 즐겨찾기/태그/컬렉션/그룹/별점 필터. MobileSheets의 Key/Genre/Difficulty 축은 Clef에서 `조성`/`장르`/`난이도`/`편성` 사용자 필드 facet으로 가볍게 흡수하고, 적용 중인 검색/필터 조건을 해제 가능한 요약 칩으로 표시 |
