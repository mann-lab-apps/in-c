...
| 라이브러리 | 여러 라이브러리 | MobileSheets 지원 | V1 | 중간 | 구현됨: library profile별 scores/setlists/view/favorite preset 저장 key 분리, 생성/전환/이름 변경/비우기. 프로필 조회는 저장하지 않으며 기본값 보정은 메모리에만 적용. 비우기는 관련 metadata/자동 백업 제거를 함께 저장하고 실패 시 롤백 시도/오류 안내 |
