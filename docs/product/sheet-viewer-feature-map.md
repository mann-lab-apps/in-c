...
| 라이브러리 | 정렬/필터 | MobileSheets 지원 | MVP | 낮음 | 14차 구현: 최근 열기/제목/작곡가/가져온 날짜 정렬, 즐겨찾기/태그/작곡가/컬렉션/그룹/별점 필터. MobileSheets의 Artists 탭은 Clef에서 작곡가 facet으로, Key/Genre/Difficulty 축은 `조성`/`장르`/`난이도`/`편성` 사용자 필드 facet으로 가볍게 흡수한다. 적용 중인 검색/필터 조건은 해제 가능한 요약 칩으로 표시하고, facet 값이 많을 때는 `더 보기` sheet에서 검색으로 좁혀 숨겨진 값까지 선택 가능 |
...
| 라이브러리 | 여러 라이브러리 | MobileSheets 지원 | V1 | 중간 | 구현됨: library profile별 scores/setlists/view/favorite preset 저장 key 분리, 생성/전환/이름 변경/비우기. 프로필 조회는 저장하지 않으며 기본값 보정은 메모리에만 적용. 비우기는 관련 metadata/자동 백업 제거를 함께 저장하고 실패 시 롤백 시도/오류 안내. 큐에 들어간 비우기 완료가 후속 편집의 화면 상태를 지우지 않도록 항목별 소유권 확인 |
...
전체 백업에 처음부터 누락으로 기록된 파일은 복원 후 누락 수를 확인 dialog로 알린다.
공유 PDF/연결 파일/필기 파일의 같은 원본 경로는 한 번만 집계한다.
복원 PDF/필기 파일은 매번 앱 내부의 새 경로에 기록하여 다른 라이브러리의 기존 파일을
덮어쓰지 않는다. 원래 파일명과 같은 백업 안의 공유 PDF 참조는 유지한다.
JSON/ZIP 복원은 metadata 쓰기/삭제의 실패 반환과 예외를 검사하고, 이미 시도한 키를 복원 직전
값으로 되돌린다. 자동 백업도 온전한 복원 상태를 한 번에 기록하며 실패 시 이전 snapshot을
유지한다. 앱 강제 종료, 지속적인 저장소 오류, 동시 변경, 실패 후 미참조 파일 정리는 별도 검토 대상이다.
앱 내 복원은 진행 창이 편집/뒤로가기/중복 진입을 막으며, 성공/취소/오류 시 조작을 다시 허용한다.
외부 공유 PDF는 복원 종료 뒤 처리하고 PDF 가져오기 중에는 백업 메뉴를 비활성화한다.
이 UI 보호를 별도 프로세스나 직접 store 호출 사이의 저장소 transaction으로 간주하지 않는다.

Songbook 곡 나누기는 해당 물리 페이지 구간의 현재 앱 필기/텍스트와 표시·공유 포함 설정도
독립 복사한다. 숨긴 페이지의 필기도 보존하고, 원본의 redo/지우기 undo 이력과 외부 필기 파일
참조는 공유하지 않는다. 분리 곡의 필기 수정은 원본 책에 반영되지 않는다.
...
- PDF 가져오기와 로컬 라이브러리.
  단일/여러 PDF, 이미지 PDF, 공유 가져오기 후 목록 저장이 실패하면 저장된 목록 복구를
  시도하고 저장 실패 안내를 표시한다. 실패한 요청은 가져오기 성공으로 안내하지 않으며,
  새 편집/라이브러리 전환을 덮지 않는다. 읽기도 실패한 경우의 복구 보장과 미참조 사본 정리는 별도다.
...
- 선택 툴바는 좁은 화면에서 선택 개수/전체 선택/세트리스트 추가를 유지하고,
  컬렉션 지정/정보 편집/제거를 더보기 메뉴로 묶는다. 넓은 화면은 기존 바로가기를 유지한다.
- 일괄 정보 편집에서 작곡가를 선택 악보에 함께 적용할 수 있다. 작곡가 입력란을 비우면
  기존 값을 유지하고, 적용한 값은 검색/작곡가 facet/저장 복원에 동일하게 반영한다.
- 명시적으로 빈 제목은 원본 파일명으로 표시하며, 라이브러리/최근 항목/세트리스트/뷰어와
  목록 복사에 같은 표시명을 쓴다. 파일명(확장자 포함) 검색과 표시명 기준 제목 정렬을 지원한다.
  빈 제목 문자열은 저장/복원 시 보존하며, 파일명으로 메타데이터를 덮어쓰지 않는다.
- 세트리스트 로드 시 제거된 곡의 참조뿐 아니라 곡별 예상 시간과 메트로놈 설정만 남은 경우도
  정리해 저장한다. 유효한 곡의 설정/이어보기 위치는 유지한다.
  정리 저장만 실패한 경우 읽은 악보와 정리된 화면은 유지하고 저장 실패를 안내한다.
  다음 로드에서 정리 저장을 재시도하며, 실제 읽기 실패와 구분한다.
  일반 악보/세트리스트의 복구 가능한 저장 경로와 시작 정리도 요청 시작 라이브러리를
  저장 계층까지 전달해 지연 성공이 새로 연 라이브러리에 기록되지 않도록 한다.
- 세트리스트 추가/제거/이름 변경/이어보기 기록은 현재 목록을 기준으로 적용한다.
  선택 창을 연 뒤 추가된 곡도 중복으로 계산해 앞선 목록 변경을 덮어쓰지 않는다.
- 대상 세트리스트가 사라지면 상세 화면은 오류 대신 찾을 수 없음 상태를 표시한다.
  일괄 추가/가져오기/곡 모음 경로도 중복이나 성공으로 안내하지 않고 다시 선택하도록 안내한다.
- 순서 입력 대기 중 세트리스트 구성/순서가 바뀌면 변경을 보류하고 다시 선택하도록 안내한다.
  드래그/위아래/직접 순번 입력에 같은 검증을 적용하며 최신 제목과 설정은 보존한다.
...
- 세트리스트 제거 되돌리기는 악보와 세트리스트가 모두 존재할 때만 복원한다. 이미 복원된 곡은
  중복 추가하지 않으며, 대상이 없어 복원할 수 없으면 이유를 안내한다.
- 세트리스트 일괄 추가는 선택 후 라이브러리에서 제거된 악보를 건너뛰고 중복 수와 별도로
  안내한다. 일부만 남았으면 유효한 악보를 추가하며, 모두 추가 불가이면 성공으로 안내하지 않는다.
  생성/복제/삭제/내용 변경 저장이 실패하면 저장된 세트리스트 목록 복구를 시도하고 새 변경은 보존한다.
  홈 일괄 추가/새 대상 생성 실패는 오류를 표시하며 선택을 유지한다. 복구 읽기 실패는 보장 범위 밖이다.
  일괄 악보 제거는 악보 목록/세트리스트 정리/자동 백업을 같은 저장 작업으로 처리한다.
  저장 대상은 제거 시작 시의 라이브러리로 고정해 대기 중 라이브러리 전환과 섞이지 않게 한다.
  저장 실패 시 기존 메타데이터 복구를 시도하며 선택을 유지한다. 원본 PDF는 삭제하지 않는다.
  상세 편집/정렬/제거/되돌리기는 저장 오류와 대상 없음/순서 충돌을 구분한다. 실패한 삭제는
  화면을 닫지 않으며, 종료한 상세 화면에는 늦은 저장 오류를 표시하지 않는다.
  가져오기 후 세트리스트 추가만 실패하면 라이브러리의 악보는 유지하고 부분 실패를 안내한다.
  대상 선택 취소/추가 실패 시 뷰어로 자동 이동하지 않으며, 목록에서 생성 실패도 오류로 안내한다.
  중복 PDF를 세트리스트에 담을 때 대상 선택 전에 열기 안내를 표시하지 않고 추가 결과만 안내한다.
  PDF/일괄 PDF/이미지/공유 가져오기 대기 중 라이브러리를 바꾸면 이전 결과를 새 목록에 넣거나
  화면 진입/오류 안내로 전달하지 않는다. 이미 시작한 저장은 원래 라이브러리에 완료한다.
  곡 모음 세트리스트 생성/추가 저장 실패는 만들어진 곡 항목을 유지하고 `다시 시도`로
  재시도한다. 화면 종료/다른 화면 진입/라이브러리 전환 뒤 후속 저장이나 안내를 이어가지 않는다.
  북마크 곡 분할의 저장 실패는 현재 요청이 소유한 목록만 복구하며 임시 곡 카드를 성공으로
  안내하지 않는다. `다시 시도` 후 정상 생성된 곡을 세트리스트로 연결할 수 있다.
- 북마크. 2차 구현은 페이지 anchor 저장, 목록 이동, 이름 변경, 삭제, PDF 목차 병합, CSV 북마크
  가져오기다.
  페이지 이동/최근 열기 기록은 최신 악보에만 적용해 그 사이 변경한 필기/정보/설정을 보존한다.
  최근 열기 기록 저장 실패는 안내하되 유효한 악보 진입/다음 곡 이동을 막지 않는다. 저장을
  기다리는 사이 화면/라이브러리/대상 곡이 바뀌면 이전 요청으로 뒤늦게 뷰어를 열지 않는다.
  이전 화면 객체의 페이지와 같더라도 현재 페이지와 다르면 돌아온 위치를 저장한다.
  악보 정보 편집은 최신 악보의 정보 필드만 갱신해 페이지/필기/설정을 보존한다. 대상이 없어졌으면
  저장 실패를 안내하며 편집창의 입력 컨트롤러는 종료 애니메이션이 끝난 뒤 해제한다.
...
- 일반 metadata 저장과 자동 백업은 쓰기/삭제 실패를 호출자에게 전달하고 이전 저장값 복구를
  ...
  다음 저장을 덮지 않게 한다. 프로세스 강제 종료나 지속적인 저장소 오류의 완전 복구는 보장하지 않는다.
  개별 악보 편집 저장이 실패하면 저장된 목록을 다시 읽어 화면 상태 복구를 시도한다.
  일괄 정보 편집과 컬렉션 지정도 같은 복구 정책을 적용하고, 실패 안내 후 선택 상태를 유지해
  재시도할 수 있다. 변경 대상이 없는 일괄 작업은 진행 중인 다른 저장 복구를 무효화하지 않는다.
  더 최신 편집/삭제/라이브러리 전환이 있으면 늦은 실패 응답으로 그 상태를 덮지 않는다.
  필기 취소/다시 적용/텍스트 수정·삭제는 저장 실패와 대상 없음을 구분해 오류 뒤에 잘못된
  빈 작업 안내를 띄우지 않는다. PDF 준비 전 취소/다시 적용은 저장된 페이지를 기준으로 처리한다.
...
- 메트로놈. visual/audible metronome, BPM/박자 저장, 악보별 metronome snapshot,
  ...
  메트로놈 창은 박자 변경을 저장 응답과 분리해 즉시 반영하고 닫기/정지 후 지연 응답이
  타이머를 재시작하지 않는다. 현재 요청의 저장 실패는 안내하되 오래된 오류는 표시하지 않는다.
