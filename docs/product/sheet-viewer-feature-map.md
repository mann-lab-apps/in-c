...
| 음악 도구 | 튜너 | Piascore 참고/차별화 | MVP | 높음 | 구현됨: `record` PCM stream, Hybrid/YIN/autocorrelation detector, RMS gate/confidence, median smoothing, no-signal debounce, octave jump 완화, note hysteresis, frequency-to-note/cents 계산, Chromatic-only 첫 화면, sharp/flat 표기 선택, A4 저장/440-442 quick action/history/보정 제안, adaptive noise floor 1차, 입력강도 bar, LED flat/center/sharp strip, 소리 작음/잡는 중/낮음/높음/맞음 feedback, needle damping/in-tune hold, 감지 엔진/debug label, 기준음/드론 연계. synthetic sine/noise/plucked string/time-series/widget test 통과. 기타 줄 맞춤/악기별 preset/custom target/target lock은 선택지 과다로 v1 UI에서 제외. 실기기 정확도/latency 검증 필요 |
...
- 튜너는 핵심 차별점이지만 전용 튜너 앱 수준의 기기별 정확도 보장은 실기기 QA 뒤에 확정한다.
  ...
  adaptive noise floor 1차, Hybrid/YIN detector 비교, 기준음/드론 연계를 제공한다. 기타 줄 맞춤,
