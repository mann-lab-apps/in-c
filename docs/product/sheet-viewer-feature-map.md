...
| 음악 도구 | 메트로놈 | 양쪽 기본 | MVP | 중간 | 구현됨: visual/audible metronome, BPM/박자 저장, 2/4·3/4·4/4·6/8, 8분/3연/16분 subdivision, 첫 박 강조, Tap tempo, 0/1/2마디 count-in, 기본 ON system click tick toggle, `소리`/`시각만` 상태 표시, viewer mini panel. 저지연 audio/accent sound asset은 후속 |
...
- 메트로놈. visual/audible metronome, BPM/박자 저장, start/stop, 첫 박 강조, subdivision,
  Tap tempo, 0/1/2마디 count-in, 기본 ON tick 소리, `소리`/`시각만` 상태 표시,
  viewer mini panel을 제공한다.
