...
| 라이브러리 metadata | 상단 탭은 Recent/Songs/Setlists/Collections/Artists/Albums/Genres. 필터는 Search, Source, Key, Collection, Difficulty, Genre, Rating. metadata 미입력 row는 제목 아래 `-`로 보인다. | title/file/source filename/최근 시간/마지막 페이지를 카드에 노출하고, 가져온 직후 `정보 편집` 안내, 홈 `정리 필요` rail, viewer `악보 정보 편집`으로 title/composer/tag/collection/group/rating/custom field를 수정할 수 있다. | MobileSheets는 import wizard의 metadata assignment가 더 강하다. Clef는 강제 wizard 대신 import 직후 nudge, 홈 review rail, viewer 안 빠른 정리 경로를 우선 제공한다. | 파일명 기반 title 추정, metadata 필터, setlist/collection 축, 악보를 보면서 정보 수정, 놓친 import 정리 재노출. | 빈 metadata `-`만 보이는 표현과 import마다 무거운 wizard 강제. | Clef는 metadata 미입력 card 식별성과 `정리 필요` rail을 유지. import-time full metadata assignment는 필요성 확인 후 검토. | v1 유지 |
...
| Import metadata assignment | MobileSheets import wizard에서 duplicate behavior와 setlist/collection/key/artist/genre 지정 가능. Clef는 로컬 PDF/이미지 import 메뉴에서 세트리스트 추가 경로를 제공하고, 같은 원본 파일명은 기존 악보로 연결하며, import 직후 `정보 편집` nudge, 홈 `정리 필요` rail, viewer 안 `악보 정보 편집`으로 정보를 바로 정리할 수 있다. | 낮음 | v1 유지 |
