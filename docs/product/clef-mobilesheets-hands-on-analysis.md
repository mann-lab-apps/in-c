...
| Import metadata assignment | MobileSheets import wizard에서 setlist/collection/key/artist/genre 지정 가능. Clef는 로컬 PDF/이미지 import 메뉴에서 세트리스트 추가 경로를 제공하고, import 직후 `정보 편집` nudge와 viewer 안 `악보 정보 편집`으로 정보를 바로 정리할 수 있다. | 중간 | v1.1 |
