...
| 메트로놈 | viewer overlay에서 metronome icon을 누르면 우측 floating panel이 뜬다. Tempo, tap tempo, time signature, subdivision, sound FX, accent first beat, playback mode, volume, start가 한 패널에 있다. | tempo, 박자, subdivision, accent, tap tempo, count-in, Android native tick volume, 악보별 metronome snapshot, 세트리스트별 tempo override, visual/tick sound, viewer mini panel과 시각 박자 strip이 있다. | MobileSheets는 설정이 풍부하지만 panel이 크고 무겁다. Clef는 실제 audio route/iOS parity QA와 advanced cue/pattern UX가 남았다. | 악보를 보면서 켜고 조정 가능한 panel, audio/visual mode 구분, count-in, volume, 곡별/세트리스트별 저장, 소리가 불분명할 때도 보이는 beat 표시. | 거대한 설정 panel과 과한 옵션 밀도. | v1은 Android tick volume과 visual fallback 유지. advanced cue/pattern은 v1.1 후보. | v1 QA, v1.1 |
