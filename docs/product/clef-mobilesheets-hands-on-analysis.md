...
| 라이브러리 metadata | 상단 탭은 Recent/Songs/Setlists/Collections/Artists/Albums/Genres. 필터는 Search, Source, Key, Collection, Difficulty, Genre, Rating. metadata 미입력 row는 제목 아래 `-`로 보인다. | title/file/source filename/최근 시간/마지막 페이지를 카드에 노출하고, viewer에서도 `악보 정보 편집`으로 title/composer/tag/collection/group/rating/custom field를 수정할 수 있다. | MobileSheets는 import wizard의 metadata assignment가 더 강하다. Clef는 강제 wizard 대신 viewer 안 빠른 정리 경로를 우선 제공한다. | 파일명 기반 title 추정, metadata 필터, setlist/collection 축, 악보를 보면서 정보 수정. | 빈 metadata `-`만 보이는 표현과 import마다 무거운 wizard 강제. | Clef는 metadata 미입력 card 식별성 계속 유지. import-time full metadata assignment는 v1.1에서 검토. | v1 유지, v1.1 |
...
| Import metadata assignment | MobileSheets import wizard에서 setlist/collection/key/artist/genre 지정 가능. Clef는 로컬 PDF/이미지 import 메뉴에서 세트리스트 추가 경로를 제공하고, viewer 안 `악보 정보 편집`으로 가져온 직후 정보를 바로 정리할 수 있다. | 중간 | v1.1 |
