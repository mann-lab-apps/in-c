...
| Import metadata assignment | MobileSheets import wizard에서 duplicate behavior와 setlist/collection/key/artist/genre 지정 가능. Clef는 로컬 PDF/이미지 import 메뉴에서 세트리스트 추가 경로를 제공하고, 같은 원본 파일명은 기존 악보로 연결하며, import 직후 `정보 편집` nudge, 홈 `정리 필요` rail, viewer 안 `악보 정보 편집`, `조성`/`장르`/`난이도`/`편성` 추천 사용자 필드와 해당 값 기반 facet으로 정보를 바로 정리하고 찾을 수 있다. | 낮음 | v1 유지 |
