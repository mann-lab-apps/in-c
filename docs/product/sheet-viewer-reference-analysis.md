...
| 메트로놈 | beat sound, first accent, count-in/stop, silent visual indicator, display mode를 제공한다. | tempo, 박자, subdivision, accent, tap tempo, count-in, 악보별 snapshot, 세트리스트별 override, mini panel, visual/tick sound, 소리 확인 버튼이 있다. | 실제 Android audio route에서 소리가 안 나는 보고가 있다. | 소리 확인 버튼과 visual-only fallback으로 볼륨/무음/이어폰 문제를 먼저 분리한다. complex pattern/autoturn 연동은 v1.1 이후. | v1 hotfix/QA |
