...
| 방식 | 장점 | MVP 판단 |
| --- | --- | --- |
| 이메일 매직링크 | 비밀번호 저장 부담이 낮고 웹/앱 공유가 쉬움 | 후속 검토 |
| 이메일/비밀번호 | 익숙하지만 재설정/보안 운영 부담이 큼 | 이후 검토 |
| OAuth | 가입 마찰이 낮고 Supabase Auth로 정적 사이트에 붙이기 쉽다 | 기본안 |
| 소셜 로그인 | 음악인 프로필과 연결 가능 | Google 우선 |

## MVP 결정

첫 인증 MVP는 Supabase Auth 기반 소셜 로그인으로 시작한다. 사용자는 Google
계정으로 웹 편집기, 커뮤니티, Electron 앱의 얕은 연동을 공유한다. 카카오
로그인은 `account_email` 권한 확보 또는 별도 구현 검토 뒤 추가하고, 네이버
로그인은 Custom OAuth/OIDC 설정 검증 뒤 후속 단계에서 추가한다.
...
- 인증 API: OAuth callback, refresh, revoke
- 프론트 UX: 로그인 모달, 세션 만료, 계정 상태
- 보안 점검: token 저장, provider redirect allowlist, 세션 만료
