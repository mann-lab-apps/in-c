...
Feature: 셋잇단음표를 입력하고 해제하기
  ...
  @scenario-tuplets-group-selected-events
  ...
  @scenario-tuplets-input-at-cursor
  ...
  @scenario-tuplets-report-input-progress
  Scenario: 셋잇단음표 입력 도중에도 진행 상태를 이해할 수 있다
    Given 입력 커서에서 셋잇단음표 입력을 시작했다
    Then 상태 영역에 셋잇단음표 입력이 "0/3" 진행됐음이 표시된다
    When 사용자가 첫 번째 음표를 입력한다
    Then 상태 영역에 "1/3" 진행과 두 개를 더 입력하라는 안내가 표시된다
    When 사용자가 두 번째 음표를 입력한다
    Then 상태 영역에 "2/3" 진행과 한 개를 더 입력하라는 안내가 표시된다
    When 사용자가 세 번째 음표를 입력한다
    Then 셋잇단음표 입력 완료 안내가 표시된다
    And 진행 안내가 사라지고 일반 입력 상태로 돌아간다
    And 진행과 완료 안내는 화면 읽기 도구가 상태 변화로 인식할 수 있다

  @scenario-tuplets-remove-existing-group
  ...
  @scenario-tuplets-reject-insufficient-time
  ...
  @scenario-tuplets-reject-relation-breaking-edit
