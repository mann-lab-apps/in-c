...
Feature: 시작화면과 복구본으로 작업을 시작하기
  사용자는 앱을 열었을 때 새 악보, 가져오기, 예제 악보, 최근 파일, 복구본 중
  ...
  Scenario: 앱 실행 후 시작화면을 본다
    ...
    When 앱 창이 열린다
    Then 사용자는 시작화면을 본다
    And 시작화면에는 새 악보 만들기, MusicXML 가져오기, 예제 악보 열기가 표시된다
    And 최근 MusicXML 파일 영역이 표시된다
    And 복구 가능한 작업이 있으면 복구본 열기가 표시된다

  Scenario: 예제 악보를 열어본다
    Given 앱이 시작화면을 보여준다
    When 사용자가 예제 악보 열기를 선택한다
    Then 앱은 번들 예제 악보를 연다
    And 예제 악보의 첫 이벤트가 선택된다
