...
Feature: 범위를 선택해 편집하기
  ...
  Scenario: 범위 선택 상태에서 복사하거나 붙여넣는다
    ...
    Then 선택 범위가 앱 내부 clipboard에 복사된다
    When 사용자가 같은 길이의 다른 단순 범위를 선택하고 Cmd 또는 Ctrl+V를 누른다
    Then 복사한 이벤트가 대상 범위를 대체한다
    And 각 마디의 전체 박자 길이는 변하지 않는다
  ...
  Scenario: 범위 선택 상태에서 음표를 쉼표로 일괄 변환한다
    ...
    When 사용자가 R을 누른다
    Then 선택 범위 안의 음표는 같은 길이의 쉼표로 바뀐다
    And 기존 쉼표는 그대로 유지된다
    And 각 마디의 전체 박자 길이는 변하지 않는다
  ...
  Scenario: 안전하지 않은 범위 일괄 변환은 거부한다
    ...
    When 사용자가 R을 누른다
    Then 악보는 변경되지 않는다
    And 사용자는 단순 범위만 쉼표로 바꿀 수 있다는 안내를 본다
