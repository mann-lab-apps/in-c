...
| FL-036 | Exported comparison next-action copy | Copied first-listen comparison evidence included raw next-action codes but omitted the readable guidance shown in the app candidate rows | `FounderDailyPickQualitySnapshot.exportText` now includes `nextActionCopy` for each comparison candidate beside the raw `nextAction` | A copied founder review packet remains actionable without translating raw action codes, while preserving search fallback and approval limits | DONE | P1 | Keep comparison export raw/copy pairs aligned with candidate-row copy | PASS: export regression, controller suite, analyze, full suite and diff check; dated evidence | Does not approve stronger candidates, links, recordings, timing, listening or founder satisfaction |
