...
| CV1-X-PITCH-FIRST | Duration-first only | Note-input state audit | Pitch/chord preview then duration commits; mode/keyboard/toolbar agree; IME/text entry safe; existing duration-first unchanged. 2026-09-25 selected-note interval chord shortcuts improve chord stacking but do not implement pitch-first mode. | State and App voice/rest/tuplet/undo regressions |
...
| CV1-X-COMMANDS-SHORTCUTS | Help dialog only | Existing command inventory | Search and run applicable commands; keyboard focus/escape; customize shortcuts, detect conflicts and reset; persist user settings. 2026-09-25 adds visible interval chord shortcut policy and keeps duration shortcuts scoped to note-input caret state. | App commands/disabled state, conflict/IME/restore tests |
