...
| `feedback_open` | Columns 피드백 UI를 열 때 | `content_type`, `content_slug`, `content_title`, `location` | 어떤 콘텐츠에서 정성 피드백을 시도하는가 |
| `feedback_submit` | Columns 피드백 선택지를 제출할 때 | `content_type`, `content_slug`, `content_title`, `answer`, `location` | 어떤 질문이 다음 대화를 만들 수 있는가 |
