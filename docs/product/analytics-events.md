...
| `work_view` | Works 상세가 렌더링될 때 | `content_type`, `content_slug`, `content_title` | Works MVP 상세 조회 | 공개 작품 메타만 전송 |
| `work_link` | 작품 링크 클릭 | `content_type`, `content_slug`, `link_url`, `link_text` | Works 관계 탐색 | 공개 작품 slug와 링크만 전송 |
| `creator_view` | Creators 상세가 렌더링될 때 | `content_type`, `content_slug`, `content_title` | Creator MVP 상세 조회 | 공개 Creator 메타만 전송 |
| `creator_link` | Creator 링크 클릭 | `content_type`, `content_slug`, `link_url`, `link_text` | Creator 관계 탐색 | 공개 Creator slug와 링크만 전송 |
| `concert_view` | Concerts 상세가 렌더링될 때 | `content_type`, `content_slug`, `content_title` | Concerts MVP 상세 조회 | 공개 공연 메타만 전송 |
| `concert_link` | 공연 링크 클릭 | `content_type`, `content_slug`, `link_url`, `link_text` | Concerts 관계 탐색 | 공개 공연 slug와 링크만 전송 |
| `class_view` | Classes 상세가 렌더링될 때 | `content_type`, `content_slug`, `content_title` | Classes MVP 상세 조회 | 공개 클래스 메타만 전송 |
| `class_link` | Class 링크 클릭 | `content_type`, `content_slug`, `link_url`, `link_text` | Classes 관계 탐색 | 공개 클래스 slug와 링크만 전송 |
...
| `release_link` | GitHub Releases 링크 클릭 | `location`, `link_url`, `link_text` | Events by `location` | 공개 외부 링크만 전송 |
| `checksum_link` | 체크섬 링크 클릭 | `location`, `link_url`, `link_text` | Events by `location` | 공개 체크섬 링크만 전송 |
| `github_link` | GitHub 저장소 링크 클릭 | `location`, `link_url`, `link_text` | Events by `location` | 공개 외부 링크만 전송 |
| `github_issue_link` | GitHub 이슈 제보 링크 클릭 | `location`, `link_url`, `link_text` | Events by `location` | 공개 외부 링크만 전송 |
...
- GitHub Releases로 우회하는 사용자가 많다면 다운로드 카드의 신뢰 문구나 파일 안내가 부족한가?
- `github_issue_link`나 `feedback_submit`이 없다면 피드백 요청 위치가 너무 늦거나 무거운가?
