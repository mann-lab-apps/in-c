...
### 2026-09-12 Current Candidate Update

This worktree is **not Commercial V1 RC signoff**. Historical Pass rows below refer
to their cited runs, not approval of this dirty worktree or a published artifact.
The reopened local queue identified missing transposing instruments, compressed
MusicXML, actual resave authorization, async save state, fermata synchronization,
editable properties, part ordering, compact score clipping and tempo/annotation
collision. Current results are recorded in
[Continuous V1 Evidence](evidence-log.md#2026-09-12-continuous-v1-discovery-and-implementation).

Required native-dialog, external-app, listening, human PDF review, DMG/installer
and Windows gates remain open. Production dependency audit was clean; the current
development-inclusive audit has advisories and is not a new blanket security Pass.
Native format is already an explicit post-V1 product decision, not unresolved Research.
...
| `npm run verify:chromatics-v1-work-queue` | Pass | Current 22-row queue includes 11 newly discovered and completed local tasks; original manual/external rows stay open. [Current evidence](evidence-log.md#2026-09-12-continuous-v1-discovery-and-implementation) |
| `npm run verify:chromatics-musescore-parity-roadmap` | Pass | Current 23-row schema distinguishes completed local slices from External QA/Research/Postpone; MS-UX-SCOPE-002 explicitly tracks unresolved expanded UI scope. Queue drain is not V1 completion. [Current evidence](evidence-log.md#2026-09-12-continuous-v1-discovery-and-implementation) |
...
| `npm run verify:midi-fixtures` | Pass | Latest sweep: [Evidence Log](evidence-log.md#2026-09-03-automatable-v1-gate-sweep); playback/mixer automated QA includes the 2026-09-12 persisted mixer/activity slice in [Evidence Log](evidence-log.md#2026-09-12-musescore-parity-same-staff-voice-presentation-slice). External MIDI app open QA remains manual RC work. |
| packaged app smoke | Partial | 2026-09-12 macOS arm64 unpacked package/smoke passed, including MXL save/reopen/resave, selected Cello PDF and MIDI. Native-dialog, installer and Windows gates remain open. [Current evidence](evidence-log.md#2026-09-12-continuous-v1-discovery-and-implementation) |
