...
## 2026-09-25 Editing UX Slice

The current multi-window implementation is a first launcher slice: File mode can
request another Electron BrowserWindow and each renderer has independent score
state. Autosave/recovery still uses the existing app-level bridge and needs
document/window-scoped storage, recovery selection, aggregate quit prompts and
packaged multi-window smoke before `CV1-X-MULTI-WINDOW` can be considered
complete.

Measure clipboard currently copies/cuts/pastes one aligned full-score measure's
voice contents: notes, rests, chords, tuplets and note-attached data inside those
events receive fresh IDs. Score-level objects anchored to measures or events
such as dynamics, text, rehearsal marks, harmonies, slurs, hairpins, octave
shifts, repeats, voltas and structural clef/key/time boundary policies are not
yet fully covered by the measure clipboard. Those remain Required follow-up, not
manual-QA-only work.
