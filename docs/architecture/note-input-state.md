...
같은 이벤트가 여러 상태에 동시에 속할 수 있으므로 렌더러는 다음 순서로
표시를 결정한다.

1. selection: 편집 대상이므로 음표/쉼표 자체의 색을 유지한다.
2. playback: 선택되지 않은 활성 이벤트만 재생 색으로 표시하고, 재생 위치는
   별도 playhead 선으로 표시한다.
3. input cursor: 다음 입력 위치는 selection이나 playback 색을 바꾸지 않고
   별도 cursor 선으로 표시한다.

선택된 이벤트가 재생 중인 경우에도 이벤트 색은 selection을 우선하고,
playhead가 재생 위치를 별도로 보여준다. 이 정책으로 사용자는 편집 대상,
다음 입력 위치, 재생 위치를 동시에 구분할 수 있다.
...
숫자키는 입력 커서와 선택 이벤트 양쪽에서 같은 음가 정책을 사용한다.
...
입력 커서가 열린 상태에서는 다음에 입력할 음표 또는 쉼표의 duration을
바꾼다. 음표나 쉼표가 선택된 상태에서는 선택 이벤트의 duration 변경
command를 실행한다. 변경이 뒤 음표를 침범하거나 마디를 넘으면 score를
바꾸지 않고 status 영역에 실패 이유를 표시한다.

32분음표 이상의 단축키는 MVP에서 보류한다. 한 자리 숫자 체계에서는 이미
일반적인 음가와 충돌이 커서, 32분음표 이상은 별도 확장 UI나 조합키 정책을
정한 뒤 추가한다.
