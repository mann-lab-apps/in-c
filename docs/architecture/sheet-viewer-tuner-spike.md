...
구현했다. V1 전달 전 보강으로 Concert/Bb Trumpet 표시 모드와 별개인 detection profile을
추가하고, Bb Trumpet profile에서는 실용 range와 더 엄격한 confidence/no-signal/hysteresis
정책을 적용한다. 이번 단계의 목표는 상용급 튜너 정확도 보장이 아니라, 트럼펫 연습자가
악보 viewer 안에서 바로 이해할 수 있는 note/cents 피드백을 crash 없이 받는 것이다.
...
- `SheetTunerSettings`
  - A4 기준음 저장.
  - 표시 모드는 `Concert`와 `Bb Trumpet`을 저장한다.
  - 감지 profile은 `Chromatic`과 `Bb Trumpet`을 저장한다.
...
- Viewer 튜너 UI
  ...
  - 공연 모드에서도 진입 가능.
  - 현재 음 이름, Concert/Bb Trumpet 표시 모드, concert pitch 보조 표시, cents meter,
    Chromatic/Bb Trumpet 감지 profile, A4 기준음 slider, start/stop, signal/confidence 표시.
  - listening이 아닐 때는 테스트 주파수 slider로 visual tuner 계산을 확인할 수 있다.
- Persistence
  - A4 기준음, 표시 모드, 감지 profile은 앱 전역 SharedPreferences 설정으로 저장.
...
2. 44.1kHz 입력을 우선 사용하고, Android 태블릿에서 실제 지원 sample rate와 buffer cadence를
   확인한다.
3. trumpet 실사용 범위에서 Bb Trumpet profile의 E3-C6 range와 confidence threshold가
   충분한지 확인한다.
4. cents jitter가 여전히 크면 profile별 smoothing window 또는 YIN 기반 detector를 비교한다.
5. Android 태블릿과 iPhone Simulator/실기기에서 latency, jitter, permission flow를 확인한다.

## 제외

- 고급 temperament.
- Trumpet 외 transposing instrument display.
- background listening.
- drone/reference tone playback.
