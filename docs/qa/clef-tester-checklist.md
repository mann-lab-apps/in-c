...
4. 같은 PDF를 다시 가져왔을 때 새 카드가 조용히 중복 생성되지 않고 기존 악보를 연다는 안내가 보이는지
   확인한다.
5. PDF를 열고 이전/다음 페이지, 세로 스크롤, 1페이지 보기 중 하나를 확인한다. 세로모드에서는
   상단 toolbar를 좌우로 밀어 `필기 모드`, `페이지 정리`, `공연 설정`, `공연 모드`에 접근한다.
   viewer의 `악보 정보 편집`에서 제목/작곡가/태그/컬렉션/그룹/별점을 바로 수정할 수 있는지도
   확인한다.
6. 펜/형광펜/크레셴도/디미누엔도와 `Fine`, `D.C.`, `D.S.`, `Coda`, `rit.`, `accel.` 같은 스탬프
   중 몇 가지를 짧게 필기하고 앱을 다시 열어 복원되는지 확인한다.
7. 텍스트 주석을 하나 추가하고 다시 탭해 수정 또는 삭제한다.
8. 북마크를 추가하고 북마크 목록에서 해당 페이지로 이동한다.
9. 튜너를 열었을 때 시작 버튼 없이 확대된 pitch history chart 안에 현재 음/cents/frequency/signal이 먼저 보이고, 기타 줄 맞춤이나 악기별
...
11. `세부 설정` 아래에서 sharp/flat 표기, 감지 엔진, A4 slider/history, 기준음/드론이 접근 가능한지
...
13. 메트로놈을 열어 BPM/박자, 8분/3연/16분 나눔, 0/1/2마디 카운트인, 첫 박 강조,
    ...
    이해되는지 확인한다.
14. 메트로놈/튜너 sheet의 작은 창 버튼으로 악보 위 mini panel을 띄우고 닫을 수 있는지 확인한다.
    메트로놈 mini panel은 시작/정지 버튼과 시각 박자 표시가 바로 보여야 하며, 소리가 작거나
    들리지 않는 환경에서도 현재 박을 눈으로 따라갈 수 있어야 한다.
15. viewer 첫 진입에서 왼쪽 `이전`, 가운데 `메뉴`, 오른쪽 `다음` tap zone 안내가 보이는지 확인한다.
    안내가 사라진 뒤 `터치 영역 다시 보기`를 눌러 같은 안내를 다시 볼 수 있는지도 확인한다.
16. 자동 스크롤을 시작한 뒤 수동 페이지 이동 시 정지되는지 확인한다.
17. `테스트 정보`에서 `피드백 템플릿 복사`를 눌러 양식이 복사되는지 확인한다.
18. 새 라이브러리를 만들 때 기존 이름을 다시 입력하면 중복 안내가 뜨고 `열기` action으로 기존
    라이브러리에 진입하는지 확인한다.
...
2. `악보 추가`에서 PDF 또는 이미지 PDF를 가져오며 세트리스트에 추가하는 action이 보이는지 확인한다.
3. 여러 악보를 일괄 선택해 기존 또는 새 세트리스트에 한 번에 추가한다. 이미 들어간 악보가 있으면
   중복 skip 안내가 이해되는지 확인한다.
4. 세트리스트 상세에서 drag handle로 순서를 바꾸고, 위/아래 버튼과 번호 배지 직접 순서 입력도
   보조로 동작하는지 확인한다.
