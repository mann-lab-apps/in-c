...
| 3 | 라이브러리 | 검색/정렬/필터, 즐겨찾기/고정, collection/group/rating/custom field가 유지된다. custom field에는 `조성`/`장르`/`난이도`/`편성` 추천 입력과 값 기반 빠른 필터 경로가 있다. | 찾지 못한 필터 조건, 정렬 이상 여부 |
...
- 세트리스트 상세는 검색 후 여러 악보를 체크하거나 현재 검색 결과를 전체 선택해 한 번에 추가하는 흐름과
  drag handle reorder를 지원하며 기존 위/아래 이동 버튼과 번호 배지 직접 순서 입력을 보조 수단으로 유지한다.
  metadata가 비어 있는 악보도 파일명/작곡가 subtitle로 구분하고, 제거 버튼은 즉시 반영되지만 snackbar
  `되돌리기`로 원래 위치에 복구할 수 있다.
