...
- Score/setlist별 metronome preset 저장과 count-in 이후 cue UX.
...
- v1 hotfix 반영됨: 다중 선택 강조, 선택 악보의 bulk setlist 추가, setlist drag reorder,
  ...
  subdivision/accent/tap tempo, 0/1/2마디 count-in, 고정형 mini tuner/metronome panel,
  새 metronome 기본 `소리 켬` 및 `소리`/`시각만` 상태 표시.
- v1.1 후보: score/setlist별 metronome preset 저장, 직접 순번 입력, import-time setlist
...
- 튜너/메트로놈은 악보 위 우상단 고정형 mini panel로 축소할 수 있다. drag/resize, 악보 가장자리
  visible pulse, per-score tempo 저장, 복잡한 accent pattern은 v1.1/Later로 분리한다.
