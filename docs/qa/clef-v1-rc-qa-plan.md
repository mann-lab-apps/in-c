...
| 3 | 라이브러리 | 검색/정렬/필터, 즐겨찾기/고정, collection/group/rating/custom field가 유지된다. custom field에는 `조성`/`장르`/`난이도`/`편성` 추천 입력과 값 기반 빠른 필터 경로가 있다. 적용 중인 검색/필터 조건은 요약 칩으로 보이고 개별/전체 초기화가 가능하다. | 찾지 못한 필터 조건, 정렬 이상 여부, 해제되지 않는 활성 조건 |
