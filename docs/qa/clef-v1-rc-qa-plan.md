...
## 사용자 실기기 피드백 (2026-09-15)

앞서 안내한 10~15분 smoke QA에 대한 사용자 보고이며 자동 검증 결과와 구분한다.
설치 buildCode, 기기/OS, BPM, 박자 나눔, 유선/Bluetooth 출력은 아직 확인되지 않았다.
22번 AAB 이후 로컬 변경의 검증 근거로 사용하지 않는다.

| 항목 | 사용자 관찰 | 판정 / 후속 확인 |
| --- | --- | --- |
| 메트로놈 | 빨라지면 `띠또 또 또`처럼 간격이 일정하지 않음 | ISSUE, 최우선 수정/재검증. BPM·나눔·출력 경로 확인 후 스피커/이어폰 비교 |
| 기준음/드론 | 이어폰에서 소리가 작게 느껴짐, 연습실에서 재확인 예정 | DEVICE QA, 결함 확정/자동 증폭하지 않음. 앱 음량·미디어 음량·출력 경로 기록 |
...
960회 요청의 객체 재사용과 실패/재시도/종료, PCM 범위는 별도 Kotlin 검증 PASS다.
실제 출력 간격을 측정한 결과는 아니며, 설치/실기기 청취 확인은 아직 하지 않았다.
22번 AAB에는 이 수정이 없으므로 현재 설치본에서 해결 여부를 판정하지 않는다.

S48에서 카운트인 시작 시 pulse를 이중 차감하는 별도 결함을 재현했다.
4/4 첫 박 강세가 한 박 일찍 나오거나 8분 나눔에서 간격이 절반으로 짧아졌다.
전체 창/미니 패널의 시작 시 차감을 제거했다. 가상 시계와 method-channel 호출에 대한
검증이며 실제 오디오 측정은 아니다. 사용자 카운트인 설정은 미확인으로, 원인 전체를
해결했다고 표현하지 않는다. 새 빌드의 빠른 BPM/출력 경로 비교는 DEVICE QA다.

S49는 지연 시 건너뛴 타이머 주기를 반영하지 않아 강세가 뒤처지는 경로를
주입 테스트로 재현했다. 경과한 pulse 수를 한 번에 반영하며 놓친 클릭을 반복 재생하지 않는다.
이는 복귀 시 위상 처리 수정이지 실제 출력 지터 제거의 증거가 아니다.

드론 소스 점검: 앱 기본 음량은 35%이며 Android는 STREAM_MUSIC으로 출력한다.
합성 gain은 `앱 음량 / 100 * 0.65 / 동시 음 수`로 합산 peak를 제한한다.
사용 당시 앱/기기 미디어 음량과 이어폰 경로는 미확인이다. 자동 증폭하지 않고
현재 앱 음량을 `드론 음량`과 백분율로 항상 표시했다. 연습실 재확인 시 같은
앱/기기 음량과 음높이/드론 모드에서 스피커·이어폰을 비교한다. 소스 수치가
실제 음압/체감 음량 측정은 아니며 시작/종료 잡음과 출력 품질도 실기기 확인 대상이다.
...
| 3 | 라이브러리 | 검색/정렬/필터, 즐겨찾기/고정, composer/collection/group/rating/custom field가 유지된다. custom field에는 `조성`/`장르`/`난이도`/`편성` 추천 입력과 값 기반 빠른 필터 경로가 있다. 적용 중인 검색/필터 조건은 요약 칩으로 보이고 개별/전체 초기화가 가능하다. facet 값이 많을 때 `더 보기`에서 검색으로 좁혀 숨겨진 값도 선택할 수 있다. | 찾지 못한 필터 조건, 정렬 이상 여부, 해제되지 않는 활성 조건 |
...
| 8-1 | 필기 layer 저장 안내 | 표시/공유 flag 저장 실패를 처리하고 최신 요청/화면/라이브러리에만 결과를 표시한다. | S41: 지연 성공/실패, 연속 토글, 종료/다른 화면/라이브러리 전환, 실제 preferences false/예외/재시도 로컬 검증. 실제 PDF 입력과 공유 파일 확인은 기기 QA |
