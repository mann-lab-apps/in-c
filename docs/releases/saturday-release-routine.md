...
| 순서 | 문서 | 언제 보는가 | 목적 |
| --- | --- | --- | --- |
| 1 | [`snapshot-template.md`](snapshot-template.md) | 목표모드나 PR이 많이 쌓였을 때 | 마지막 태그 이후 변경을 5-10분 안에 `완료됨`, `문서화 완료`, `제품·사이트 반영 필요`, `결정 필요`, `릴리즈 보류`로 나눈다 |
| 2 | 이 문서의 데일리 루틴 | 하루 작업을 시작하거나 마칠 때 | 자주 확인해야 할 신호와 막힌 결정을 짧게 본다 |
| 3 | 이 문서의 주간 루틴 | 매주 토요일 또는 릴리즈를 고민할 때 | 이번 주를 `진행`, `보류`, `차주 이월` 중 하나로 판단한다 |
| 4 | [`../quality/current-state-report.md`](../quality/current-state-report.md) | 현재 제품 상태가 헷갈릴 때 | 앱, 사이트, backend, known limitation의 큰 상태를 복원한다 |
| 5 | [`../quality/known-limitations.md`](../quality/known-limitations.md) | 사용자에게 말할 제한사항을 정리할 때 | 릴리즈 노트와 사이트 문구에 남겨야 할 제한을 확인한다 |
| 6 | [`../quality/release-readiness-checklist.md`](../quality/release-readiness-checklist.md) | 릴리즈 진행 가능성이 있을 때 | release candidate로 볼 수 있는지 quality 기준으로 확인한다 |
| 7 | [`checklist-template.md`](checklist-template.md) | 실제 prerelease를 만들기로 했을 때 | 태그, 패키징, 사이트 배포, 배포 후 확인을 끝까지 실행한다 |
| 8 | [`manual-score-completion-qa.md`](manual-score-completion-qa.md) | 사람이 앱을 직접 만질 때 | 한 악보 완성 흐름으로 수동 QA를 수행한다 |

평소에는 1-3번만 보면 충분하다. 릴리즈를 실제로 진행하려는 날에는 4-8번으로 내려간다.
특정 판단의 근거가 더 필요할 때만
...
데일리 점검에서 목표모드나 PR이 많이 쌓여 분류가 어려워졌다고 느끼면,
[`snapshot-template.md`](snapshot-template.md)를 사용해 작은 스냅샷을 만든다. 이때도
완료본을 저장소에 커밋할 필요는 없다. 릴리즈 판단이나 외부 공유가 필요할 때만
기록으로 남긴다.
...
목표모드나 PR이 많이 쌓여 이번 주 변경을 한 번에 파악하기 어렵다면, 먼저
[`snapshot-template.md`](snapshot-template.md)로 마지막 태그 이후 변경을 5-10분 안에
분류한다. 이 스냅샷은 토요일 루틴을 대체하지 않고, 1번 `변경 범위` 확인의 입력으로
사용한다.
...
| 관점 | 막힌 지점 | 문서 반영 |
| --- | --- | --- |
| 최근 `main` 변경 범위 | `v0.1.0-alpha.4..HEAD` 변경 파일이 많아 후보/제외 구분이 먼저 필요했다 | 스냅샷을 변경 범위 입력으로 명시 |
...
| 사용자-facing 영향 | PR #383처럼 문서 변경은 닫힌 이슈가 많아도 화면 영향이 없을 수 있다 | snapshot 분류와 release note 후보를 함께 본다 |
| 문서화 완료와 제품 반영 필요 | 기준 문서가 생긴 것을 제품 완료로 착각할 수 있다 | 스냅샷 분류를 주간 루틴 앞에 둔다 |
| 데일리와 주간 연결 | 데일리 이상 신호가 주간 판정으로 넘어가는 경로가 약했다 | 데일리에서 분류가 어려워지면 스냅샷을 쓰도록 명시 |
