...
이 문서는 릴리즈 전에 프로젝트 현황을 5-10분 안에 복원하기 위한 얇은 현황판이다.
상시 운영 보드가 아니며, 매 목표모드나 매 PR마다 완료본을 커밋하지 않는다.
릴리즈를 진행할지 판단해야 할 때만 이 템플릿을 PR, 이슈, GitHub Release 준비
메모, 또는 `docs/releases/checklists/`의 저장본 앞부분에 붙여 사용한다.
...
이 스냅샷은 [`saturday-release-routine.md`](saturday-release-routine.md)의 30분 점검을
대체하지 않는다. 스냅샷에서 `진행` 판정을 내린 뒤 실제 릴리즈를 준비할 때는
[`checklist-template.md`](checklist-template.md)를 사용한다.
