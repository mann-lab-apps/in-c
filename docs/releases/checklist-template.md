...
토요일 주간 점검에서 릴리즈를 진행하기로 판단한 경우
`docs/releases/saturday-release-routine.md`의 기록을 이 체크리스트의 시작점으로 삼는다.
...
- macOS prerelease는 Developer ID 서명과 notarization을 적용한다.
- Windows code signing 인증서를 설정하기 전까지 Windows SmartScreen 안내를
  숨기지 않는다.
...
- [ ] macOS 릴리즈용 GitHub secret `MACOS_CSC_LINK`, `MACOS_CSC_KEY_PASSWORD`,
  notarization credential 조합이 설정되어 있다.
...
- [ ] GitHub Actions `Release` workflow의 macOS, Windows, Linux package job이 모두
  통과했다.
- [ ] macOS package job에서 Developer ID signing, notarization, stapling이 성공했다.
...
- [ ] `npm run verify:site-production`으로 공식 도메인, GitHub Pages fallback,
  sitemap, robots, 다운로드 manifest, TLS 상태를 확인했다.
...
- [ ] macOS artifact를 설치한 뒤 Gatekeeper 경고 없이 실행되는지 확인했다.
- [ ] Windows code signing 전까지 SmartScreen 안내 문구가 보이는지 확인했다.
